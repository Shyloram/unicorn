...
#ifndef __RW_CONFIG_INTERFACE__
#define __RW_CONFIG_INTERFACE__
...
#ifdef ZMD_APP_DEBUG_RWC
#define RWCLOG(format, ...)     fprintf(stdout, "[RWCLOG][Func:%s][Line:%d], " format, __FUNCTION__,  __LINE__, ##__VA_ARGS__)
#else
#define RWCLOG(format, ...)
#endif
#define RWCERR(format, ...)     fprintf(stdout, "\033[31m[RWCERR][Func:%s][Line:%d], " format"\033[0m", __FUNCTION__,  __LINE__, ##__VA_ARGS__)

#define resp1_len 816
#define resp2_len 1736-252-264-84

#ifndef CHANNEL_PARA
#define	CHANNEL     	   16
#endif

#ifndef PARTITION_MAXNUM
#define PARTITION_MAXNUM	4
#endif

#ifndef ALARMOUTPORTNUM
#define	ALARMOUTPORTNUM	8
#endif

#ifndef WEBSEVER_SCHEDULE_MAX_NUM
#define WEBSEVER_SCHEDULE_MAX_NUM		10 /* 平台最大支持时间段个数*/
#endif 

#ifndef MAX_ALARM_ZONE
...
typedef struct _MotAlarmStrategy
{	
	int cloud_m_video_last;	
	int cloud_s_video_last;	
	p2p_alarm_stream_level sl[20];	
	int sl_size;
}MotAlarmStrategy;

typedef struct _PirAlarmStrategy{
	int interval;
	int video_last;
	int s_time;
	int cloud_m_video_last;
...
typedef struct 
{
	unsigned char voice_message_index;			//用户正在使用哪条留言 0-4
	unsigned char chime;						//按门铃时是否响铃
	unsigned char cvr_on;						//云存储是否开
	unsigned char use_voice_message;			//是否使用语音留言
	unsigned char mute;           				//1打开输入声音 0 关闭输入声音
	unsigned int  cvr_timeout_intv;				//云存储报警间
	unsigned char device_backlight;			//背光
	unsigned int  synckey;						//跟服务器同步的序列号
	unsigned char nightvision_switch; 		//夜视开关 1:自动 2：开启红外 3：关闭红外
	unsigned char imageflip_switch;			//图像控制 0：正常 1：翻转 2：镜像 3：180度旋转
	short   ircut_admax;
	short   ircut_admin;
	unsigned char sound_detetion;		//声音报警开关 0开  1关
	unsigned char sound_sensitivity;		//声音报警灵敏度 0最高 1中等 2最低
    char          reserved[82];      			//保留
}DoorbellPara;
...
typedef struct
{
    ...
    MasterInfo              m_Master;       /*32Bytes主人信息*/
	char 				    m_reserved[72];
	//DoorbellPara			m_doorbell;		/*72+32Bytes门铃参数*/
...
typedef struct 
{
	int index; // 预置点的编号, 非负数
	char name[48]; // 名称
	int status; // 0, 关闭; 1, 启用
	char bindDevice[16]; // 关联的被动设备id
	int position0; // 电机1距离限位开关的步数
	int position1; // 电机2距离限位开关的步数
} PresetPointInfo; 

/* Pivot预置点属性 */
typedef struct 
{
	PresetPointInfo    		m_presetPointInfo[MAX_SD433];
} PresetPointSet; //2560字节

typedef struct _MD_Region {
    float x;      //横坐标比
    float y;      //纵坐标比
    float width;  //宽比
    float height; //高比
...
typedef struct 
{
	...
	PresetPointSet				m_presetPointSet;
	SubDevices              m_subDev;           //被动设备列表  768字节 		
	char 					m_res8[resp1_len];		
	...
	MotAlarmStrategy   		m_MotAlarmStrategy;  //252
	PirAlarmStrategy		m_PirAlarmStrategy;		//264
	MD_Region				m_MdRegion;			//84
	char					m_res[resp2_len];		//	2052+60-360 =1752-16=1736
	int                     m_have_flash_init;
...
#define CONFIG_PARAMETER_ADDRESS			 (0x800000)
#define CONFIG_PARAMETER_SIZE				 (0x20000)  //200k

#define CONFIG_PARAMETER_TAIL CONFIG_PARAMETER_ADDRESS + CONFIG_PARAMETER_SIZE
#define USING_SPI_FLASH 1                            //1 using spi flash ,else using nand flash

#define CONFIG_SPI_FLASH_PAGE_SIZE 256
//dolphin 20180613 modify: block erase start addr original should be times of 64k ,now change to 4k
#define ERASE_FLASH_BLOCK_SIZE   65536    //64*1024   
//#define PAGE_CORRECT_IN_USE             //if define then use correct num to page size
//#define BLOCK_CORRECT_IN_USE
#define FLASH_HAVE_INIT_VALUE 666

/*
int HandleConfigParam(CONFIG_PARA cmd, void *paraDate)

cmd.method:
	CONFIG_READ            = 0  ,        //read from config (partial)
	CONFIG_WRITE           = 1  ,        //write to config (partial)        
	CONFIG_CLEAR_PART      = 2  ,
	CONFIG_CLEAR_ALL       = 3  ,
	CONFIG_READ_ALL        = 4  ,        //read all 

cmd.paraDate: input/ouput data

this function is written  for read/write/clear spi/nand flash for system init

example:
	CONFIG_PARA cmd;
	NETWORK_PARA t;
	cmd.method = CONFIG_WRITE;
	cmd.category = CONFIG_NETWORK_PARA;
	t.m_changeinfo = 1;
	HandleConfigParam(cmd,&t);

How to add an member:
1) SYSTEM_PARAMETER   add your member
2) enum argmethod     add your member
3) class RWConfig     add your member
...
/*********************** method **********************/
//for reentrant ,so here use define instead of enum
enum CONFIG_METHOD
{
	CONFIG_READ        = 0 ,        //read from config (partial)
	CONFIG_WRITE           ,        //write to config (partial)        
	//CONFIG_CLEAR_PART      ,      //dolphin no use 
	CONFIG_CLEAR_ALL       ,
	CONFIG_READ_ALL        ,        //read all 
	CONFIG_INIT_DEFAULT    ,
};
...
enum argmethod
{
	CONFIG_NETWORK_PARA          =  0 , //NETWORK_PARA
	CONFIG_MACHINE_PARA          =  1 , //MACHINE_PARA
	CONFIG_CAMERA_PARA           =  2 , //CAMERA_PARA
	CONFIG_CAMERA_ANALOG         =  3 ,	 //CAMERA_ANALOG
	CONFIG_CAMERASENSOR_PARA     =  4 ,	 //CAMERASENSOR_PARA
	CONFIG_COMMON_PARA           =  5 , //COMMON_PARA
	CONFIG_CAMERA_BLIND          =  6 ,	 //CAMERA_BLIND
	CONFIG_ALARM_OUT_PORT        =  7 ,	 //AlarmOutPort
	CONFIG_CAMERA_VIDEO_LOSS     =  8 ,  //CAMERA_VideoLoss
	CONFIG_CAMERA_MD             =  9 ,	 //CAMERA_MD
	CONFIG_PTZ_PARA              =  10, //PTZ_PARA	
	CONFIG_USER_GROUP_SET        =  11, //USERGROUPSET	
	CONFIG_SYSTEM_MAINETANCE     =  12, //SYSTEMMAINETANCE
	CONFIG_VIDEO_DISPLAY_SET     =  13, //VIDEODISPLAYSET
	CONFIG_GROUP_EXCEPT_HANDLE   =  14, //GROUPEXCEPTHANDLE
	CONFIG_PRESET_POINT_SET      =  15, //PresetPointSet
	CONFIG_SUB_DEVICE            =  16, //SubDevices
	CONFIG_RESPONSE_1            =  17, //char []816
	CONFIG_VIDEO_OSD_INSET       =  18, //VIDEOOSDINSERT
	CONFIG_GROUP_RECORD_TASK     =  19, //GROUPRECORDTASK
	CONFIG_ALARM_ZONE_GROUP_SET  =  20, //ALARMZONEGROUPSET
	CONFIG_PARA_METER_EXTEND     =  21, //PARAMETEREXTEND
	CONFIG_WEB_SYNC_PARAM        =  22, //web_sync_param_t
	CONFIG_P2P_MD_REGION_CHANNEL =  23, //P2P_MD_REGION_CHANNEL
	CONFIG_MOTOR_ALARM_STRATEGY  =  24, //MotAlarmStrategy
	CONFIG_PIR_ALARM_STRATEGY    =  25, //PirAlarmStrategy
	CONFIG_MD_REGION             =  26, //MD_Region
	CONFIG_RESPONSE_2            =  27, //char [] 1736-252-264-84
	CONFIG_FLASH_HAVE_INIT_BIT   =  28, //int flag ,if flash have been init ,then set 666 ,else set 0
	CONFIG_METHOD_MAX,
};
...
{	
	private:
		static RWConfig* m_instance;
		static pthread_mutex_t mutex;
		static SPENODE addrTable[CONFIG_METHOD_MAX];
		static int MAX_SPI_PARA_ADDRESS;

	private:
		RWConfig();
		~RWConfig();
		void initForusing(void);

	public: 
		static RWConfig* GetInstance();
		void init(void);
		int handle(CONFIG_PARA t,void *arg);
		void release(void);
		void Zmodo_Init(void);
};
